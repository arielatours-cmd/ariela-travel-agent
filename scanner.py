...
def _fallback_bag_fee(airline, kind):
    if not airline: return None
    row=AIRLINE_BAGGAGE_FALLBACK_ILS.get(str(airline).strip().upper(), {})
    v=row.get(kind)
    return float(v) if isinstance(v,(int,float)) and v>0 else None

def _conservative_rt_bag_fee(out_fee, ret_fee, out_airline, ret_airline, kind):
    """Higher known/fallback one-way fee x2, intentionally conservative."""
    vals=[v for v in (out_fee,ret_fee) if isinstance(v,(int,float)) and v>=0]
    if not vals:
        vals=[v for v in (_fallback_bag_fee(out_airline,kind),
                          _fallback_bag_fee(ret_airline,kind)) if v is not None]
