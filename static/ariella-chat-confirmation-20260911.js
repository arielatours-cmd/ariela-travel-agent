(function(){
  function boot(){
    ...
    function saveState(){try{localStorage.setItem(STORAGE_KEY,JSON.stringify({profile,history:history.slice(-80),assistanceChoices,assistanceAsked,saved_at:new Date().toISOString()}));}catch(e){}}
    ...
    function renderAssistance(){
      ...
      card.innerHTML=`<strong>במה תרצי שאסייע בהמשך תכנון החופשה?</strong><div class="ac-help-options"><label class="ac-help-option"><input type="checkbox" value="route"> תכנון מסלול</label><label class="ac-help-option"><input type="checkbox" value="attractions"> אטרקציות</label><label class="ac-help-option"><input type="checkbox" value="lodging"> מקומות לינה</label><label class="ac-help-option"><input type="checkbox" value="car"> השכרת רכב</label></div><div class="ac-help-actions"><button type="button" class="ac-help-save">הצגת תוצאות</button><button type="button" class="ac-help-later">לא כרגע</button></div><div class="ac-help-results" hidden></div>`;
      messages.appendChild(card);assistanceChoices.forEach(v=>{const el=card.querySelector(`input[value="${v}"]`);if(el)el.checked=true;});
      const results=card.querySelector('.ac-help-results');
      card.querySelector('.ac-help-save').addEventListener('click',()=>{assistanceChoices=[...card.querySelectorAll('input:checked')].map(x=>x.value);profile.assistance_choices=assistanceChoices;saveState();results.hidden=false;if(!assistanceChoices.length){results.textContent='לא נבחר תחום.';return;}const parts=[];if(assistanceChoices.includes('attractions')){if(lastAttractions.length)parts.push('<b>אטרקציות מתאימות:</b><ul>'+lastAttractions.slice(0,5).map(x=>`<li>${x.name}${x.city?' — '+x.city:''}</li>`).join('')+'</ul>');else parts.push('<b>אטרקציות:</b> לא נמצאו כרגע תוצאות מתאימות במאגר.');}if(assistanceChoices.includes('route'))parts.push('<b>תכנון מסלול:</b> אין כרגע תוצאות מסלול מוכנות.');if(assistanceChoices.includes('lodging'))parts.push('<b>מקומות לינה:</b> אין כרגע תוצאות זמינות בתחום זה.');if(assistanceChoices.includes('car'))parts.push('<b>השכרת רכב:</b> אין כרגע תוצאות זמינות בתחום זה.');results.innerHTML=parts.join('<div style="height:8px"></div>');messages.scrollTop=messages.scrollHeight;});
      card.querySelector('.ac-help-later').addEventListener('click',()=>{assistanceChoices=[];profile.assistance_choices=[];saveState();card.remove();});messages.scrollTop=messages.scrollHeight;
    }
    async function submit(){
      const raw=input.value.trim();if(!raw||busy)return;
      busy=true;input.value='';send.disabled=true;add('user',raw);
      ...
      try{
        ...
        if(data.show_assistance&&!assistanceAsked){assistanceAsked=true;renderAssistance();}
        saveState();
    ...
    else {const welcome='היי, אני אריאלה. איזו חופשה אתם מחפשים?';add('bot',welcome);history=[{role:'assistant',content:welcome}];saveState();}
    summary();if(assistanceAsked)renderAssistance();input.placeholder='';
  }
