(function(){
  function boot(){
    ...
    const OLD_KEYS=['ariellaChatState:v1','ariellaChatState:v2'];
    ...
    function add(who,text){
      ...
      if(who==='bot'){const a=document.createElement('div');a.className='ac-mini-avatar';a.textContent='A';row.appendChild(a)}
      const b=document.createElement('div');b.className='ac-bubble';b.textContent=text;row.appendChild(b);messages.appendChild(row);messages.scrollTop=messages.scrollHeight;
    }
    ...
    function renderAssistance(){
      ...
      card.innerHTML=`<strong>במה תרצי שאסייע בהמשך תכנון החופשה?</strong><div class="ac-help-options">
      ...
      </div><div class="ac-help-actions"><button type="button" class="ac-help-save">הצגת תוצאות</button><button type="button" class="ac-help-later">לא כרגע</button></div><div class="ac-help-results" hidden></div>`;
      messages.appendChild(card);
      assistanceChoices.forEach(v=>{const el=card.querySelector(`input[value="${v}"]`);if(el)el.checked=true;});
      ...
      card.querySelector('.ac-help-save').addEventListener('click',()=>{
        ...
        if(!assistanceChoices.length){results.textContent='לא נבחר תחום.';return;}
        const parts=[];
        if(assistanceChoices.includes('attractions')){
          if(lastAttractions.length){parts.push('<b>אטרקציות מתאימות:</b><ul>'+lastAttractions.slice(0,5).map(x=>`<li>${x.name}${x.city?' — '+x.city:''}</li>`).join('')+'</ul>');}
          else parts.push('<b>אטרקציות:</b> לא נמצאו כרגע תוצאות מתאימות במאגר.');
        }
        if(assistanceChoices.includes('route'))parts.push('<b>תכנון מסלול:</b> אין כרגע תוצאות מסלול מוכנות.');
        if(assistanceChoices.includes('lodging'))parts.push('<b>מקומות לינה:</b> אין כרגע תוצאות זמינות בתחום זה.');
        if(assistanceChoices.includes('car'))parts.push('<b>השכרת רכב:</b> אין כרגע תוצאות זמינות בתחום זה.');
        results.innerHTML=parts.join('<div style="height:8px"></div>');
        messages.scrollTop=messages.scrollHeight;
      });
      card.querySelector('.ac-help-later').addEventListener('click',()=>{assistanceChoices=[];profile.assistance_choices=[];saveState();card.remove();});
      messages.scrollTop=messages.scrollHeight;
    }
    async function submit(){
      const raw=input.value.trim();if(!raw||busy)return;
      busy=true;input.value='';send.disabled=true;add('user',raw);const priorHistory=history.slice(-12);
      try{
        ...
        if(!res.ok||data.status!=='success'){add('bot',aiErrorMessage(data));return;}
        profile=data.profile||profile;lastAttractions=data.travel_agent?.attractions||[];summary();
        const reply=String(data.reply||'').trim();
        if(reply)add('bot',reply);
        history.push({role:'user',content:raw});
        if(reply)history.push({role:'assistant',content:reply});
        chat.dataset.tinkerbellHandoff=JSON.stringify(data.tinkerbell_handoff||{});
        chat.dataset.readyForFlights=data.ready_for_flights?'1':'0';
        chat.dataset.flightSearchStarted=data.flight_search_started?'1':'0';
        if(data.show_assistance&&!assistanceAsked){assistanceAsked=true;renderAssistance();}
        saveState();
      }catch(e){add('bot','יש כרגע תקלה זמנית. נסי שוב בעוד רגע.');}
    ...
    else add('bot','היי, אני אריאלה. איזו חופשה אתם מחפשים?');
    summary();
