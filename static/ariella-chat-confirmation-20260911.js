(function(){
  function boot(){
    ...
    async function submit(){const raw=input.value.trim();if(!raw||busy)return;busy=true;input.value='';send.disabled=true;add('user',raw);history.push({role:'user',content:raw});try{const res=await fetch('/api/ariella/chat',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'same-origin',body:JSON.stringify({message:raw,history:history.slice(-12),profile})});const data=await res.json();if(!res.ok||data.status!=='success'){add('bot',aiErrorMessage(data));return;}profile=data.profile||profile;summary();const reply=data.reply||'ספרו לי עוד קצת על החופשה שאתם מחפשים.';add('bot',reply);history.push({role:'assistant',content:reply});chat.dataset.tinkerbellHandoff=JSON.stringify(data.tinkerbell_handoff||{});chat.dataset.readyForFlights=data.ready_for_flights?'1':'0';}catch(e){add('bot','יש כרגע תקלה זמנית בחיבור לאריאלה. אפשר לנסות שוב בעוד רגע.');}finally{busy=false;send.disabled=false;input.focus()}}
