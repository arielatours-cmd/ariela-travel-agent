(function(){
  function boot(){
    ...
    if(title)title.textContent='טינקרבל — סוכנת הנסיעות הראשית שלך';
    chat.querySelectorAll('.ac-avatar,.ac-mini-avatar').forEach(x=>x.textContent='T');
    const note=chat.querySelector('.ac-note');if(note)note.textContent='טינקרבל מתשאלת • אריאלה מחפשת טיסות • Travel מתאים אטרקציות';
    ...
    function add(who,text){const row=document.createElement('div');row.className='ac-row '+who;if(who==='bot'){const a=document.createElement('div');a.className='ac-mini-avatar';a.textContent='T';row.appendChild(a)}const b=document.createElement('div');b.className='ac-bubble';b.textContent=text;row.appendChild(b);messages.appendChild(row);messages.scrollTop=messages.scrollHeight;}
    ...
    async function submit(){const raw=input.value.trim();if(!raw||busy)return;busy=true;input.value='';send.disabled=true;add('user',raw);history.push({role:'user',content:raw});try{const res=await fetch('/api/tinkerbell/chat',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'same-origin',body:JSON.stringify({message:raw,history:history.slice(-12),profile})});const data=await res.json();if(!res.ok||data.status!=='success')throw new Error(data.message||'chat error');profile=data.profile||profile;summary();const reply=data.reply||'ספרו לי עוד קצת על החופשה שאתם מחפשים.';add('bot',reply);history.push({role:'assistant',content:reply});const travelMsg=attractionsText(data.travel_agent?.attractions||[]);if(travelMsg)add('bot',travelMsg);chat.dataset.ariellaHandoff=JSON.stringify(data.ariella_handoff||{});chat.dataset.readyForFlights=data.ready_for_flights?'1':'0';}catch(e){add('bot','יש כרגע תקלה זמנית בחיבור לטינקרבל. אפשר לנסות שוב בעוד רגע.');}finally{busy=false;send.disabled=false;input.focus()}}
    ...
    if(first)first.textContent='היי, אני טינקרבל, סוכנת הנסיעות הראשית של אריאלה. ספרו לי חופשי איזו חופשה אתם מחפשים — גם אם עדיין אין לכם יעד. אני אשאל רק מה שחסר, וברקע אעביר לאריאלה את פרטי הטיסה ול-Travel את ההעדפות למסלול ולאטרקציות.';
