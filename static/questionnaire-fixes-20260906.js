(function(){
  function init(){
    ...
    if(form){
      ...
      function addVisual(input,icon,extraClass){const label=input?.closest('.choice-button'),span=label?.querySelector(':scope > span');if(!label||!span)return;const i=document.createElement('i');i.className='answer-visual-icon '+(extraClass||'');i.setAttribute('aria-hidden','true');i.textContent=icon;span.prepend(i);label.classList.add('has-answer-visual');}
