(function(){
  function init(){
    ...
    if(form){
      ...
      // Keep budget wording clean and identical across Hebrew/English and mobile/desktop.
      ...
      [
        form.querySelector('.wizard-step[data-route="standard"][data-step="5"]'),
        form.querySelector('.business-question[data-step="6"]'),
        form.querySelector('.ski-question[data-step="7"]')
      ].forEach(step=>{const h=step?.querySelector('h2');if(h)h.textContent=isEn?'Would you like to set a budget?':'האם תרצו להגביל את התקציב?';});

      // Add subtle visual cues to common answer choices.
      const icons={
        destination_mode:{specific:'📍',open:'✦'},
        date_mode:{exact:'▣',month:'▦',anytime:'↔'},
        budget_mode:{per_person:'₪',unlimited:'▤'},
        business_budget_mode:{per_person:'₪',unlimited:'▤'},
        ski_budget_mode:{per_person:'₪',unlimited:'▤'},
        deal_priorities:{direct:'✈',baggage:'▣',dates:'▦',maximize:'↗'},
        ski_date_mode:{exact:'▣',month:'▦',ski_flexible:'↔'}
      };
      Object.entries(icons).forEach(([name,values])=>Object.entries(values).forEach(([value,icon])=>{
        form.querySelectorAll(`input[name="${name}"][value="${value}"]`).forEach(input=>{
          const label=input.closest('.choice-button'); if(!label||label.querySelector('.choice-icon'))return;
          const span=label.querySelector(':scope > span'); if(!span)return;
          const i=document.createElement('i');i.className='choice-icon';i.setAttribute('aria-hidden','true');i.textContent=icon;span.prepend(i);
        });
      }));

      // Back to vacation type must clear the selected vacation type so the same type can be chosen again.
      form.querySelectorAll('.wizard-type-back').forEach(button=>button.addEventListener('click',()=>{
        form.querySelectorAll('input[name="vacation_type"]').forEach(input=>{
          input.checked=false;
          input.closest('.vacation-type-card')?.classList.remove('selected');
        });
      },true));
      ...
      if(timePanel&&!timePanel.querySelector('.business-time-field-headings')){
        timePanel.querySelectorAll('.two-cols > label').forEach(function(label){
          const inputs=label.querySelectorAll('input[type="date"],input[type="time"]');if(inputs.length!==2)return;
          const headings=document.createElement('div');headings.className='business-time-field-headings';
          const dateLabel=document.createElement('span');dateLabel.textContent=isEn?'Date':'תאריך';
          const timeLabel=document.createElement('span');timeLabel.textContent=isEn?'Time':'שעה';headings.append(dateLabel,timeLabel);
          inputs[0].insertAdjacentElement('beforebegin',headings);inputs[0].classList.add('business-time-date');inputs[1].classList.add('business-time-clock');
        });
      }
      ...
      function syncSoloSki(){
        ...
        const label=mixed?.closest('.choice-button');if(label)label.hidden=!!solo;
    ...
    // Account page: remove internal "initial scan" terminology from customer UI.
    document.querySelectorAll('.search-status').forEach(el=>{
      const t=(el.textContent||'').trim().toLowerCase();
      if(t==='סריקה ראשונית'||t==='initial scan')el.remove();
    });

    // WhatsApp early-deals control: restore the one-time ₪9 wording without overlapping the switch.
    document.querySelectorAll('.notification-toggle').forEach(button=>{
      if(button.querySelector('.whatsapp-one-time-price'))return;
      const text=button.querySelector(':scope > span:not(.toggle-track)');if(!text)return;
      const note=document.createElement('small');note.className='whatsapp-one-time-price';
      note.textContent=isEn?'One-time payment · ₪9':'תשלום חד־פעמי · 9 ₪';
      text.appendChild(note);
    });

    // Restore the approved two personal-search plans using the existing backend plan keys.
    document.querySelectorAll('.plan-options').forEach(box=>{
      if(box.dataset.twoPlans==='1')return;
      const firstForm=box.querySelector('form');if(!firstForm)return;
      const action=firstForm.getAttribute('action')||'';
      const plans=isEn?
        [{key:'daily',name:'Database search',desc:'Smart search using Ariella’s shared deal database',price:'₪19'},{key:'intensive',name:'Intensive search',desc:'Includes external scans when needed',price:'₪39'}]:
        [{key:'daily',name:'חיפוש במאגר',desc:'חיפוש חכם במאגר הדילים המשותף של אריאלה',price:'19 ₪'},{key:'intensive',name:'חיפוש אינטנסיבי',desc:'כולל סריקות חיצוניות לפי הצורך',price:'39 ₪'}];
      box.innerHTML='';box.dataset.twoPlans='1';
      plans.forEach((p,idx)=>{
        const f=document.createElement('form');f.method='post';f.action=action;
        const input=document.createElement('input');input.type='hidden';input.name='plan';input.value=p.key;
        const b=document.createElement('button');b.type='submit';b.className='plan-choice'+(idx===0?' recommended':'');
        if(idx===0){const em=document.createElement('em');em.textContent=isEn?'Recommended':'מומלץ';b.appendChild(em);}
        const strong=document.createElement('strong');strong.textContent=p.name;
        const span=document.createElement('span');span.textContent=p.desc;
        const price=document.createElement('b');price.textContent=p.price+' · '+(isEn?'one-time payment':'תשלום חד־פעמי');
        b.append(strong,span,price);f.append(input,b);box.appendChild(f);
      });
    });
  }
