(function(){
  ...
  var FLOW_VERSION='20260914-natural-v4';
  ...
  function removeLegacyVacationGate(){
    document.querySelectorAll('#vacationTypeGate,.vacation-type-gate').forEach(function(el){el.remove();});
    document.querySelectorAll('.vacation-type-grid').forEach(function(el){if(el.closest('#tripWizard'))el.remove();});
  }
  ...
  function bootDirectChat(){
    ...
    document.querySelectorAll('.trip-back-bar').forEach(function(el){el.style.setProperty('display','none','important');});
    ...
    if(!existing){
      ...
      var script=document.createElement('script');script.src='/static/ariella-chat-confirmation-20260911.js?v=20260914-natural-v4';script.async=false;document.body.appendChild(script);
    }
    loadAvatarSource();
    enforceCurrentUi();
