...
def _safe_reply(result): return str(result.get("reply") or "").strip() or "ספרי לי עוד 😊"

@ariella_chat_v2.post("/api/ariella/chat-clean")
def ariella_chat_clean_proxy():
    return chat_clean()
...
def ariella_chat():
    ...
    merged=dict(profile); merged.update(result.get("profile_patch") or {})
    ...
    stage=_stage(merged); complete=stage=="confirm"; services=merged.get("services") or []; reply=_safe_reply(result)
    ...
    return jsonify({"status":"success","agent":"Ariella","engine_version":ENGINE_VERSION,"reply":reply,"profile":merged,"intent":result.get("intent") or "conversation","missing_question":"","stage":stage,"show_purpose_picker":False,"show_service_picker":False,"ui_choice":None,"services":services,"ready_for_flights":complete and "flight" in services,"flight_search_started":False,"ready_for_lodging":complete and "lodging" in services,"ready_for_car":complete and "car" in services,"ready_for_travel":bool(travel),"intake_complete":complete,"requires_confirmation":complete,"confirmation_text":"עברו על כל הפרטים ב'החופשה שלי'. אם הכול נכון, אשרו יציאה לחיפוש.","tinkerbell_handoff":_flight_handoff(merged),"travel_agent":{"attractions":travel},"lodging_schema":_load_json(_LODGING_SCHEMA_FILE,{}) if complete and "lodging" in services else {},"car_schema":_load_json(_CAR_SCHEMA_FILE,{}) if complete and "car" in services else {},"inventory_status":{"lodging":lodging_status,"car":"provider_pending"}})
