...
## Product decision update — number-first identity (N2 final)
- Verified WhatsApp sender number is the primary identity source.
- Browser/login state must never determine WhatsApp identity.
- On inbound message: existing WhatsApp link -> normalized members.phone lookup -> short onboarding/new account.
- The website button opens a generic "היי אריאלה 👋" message; no browser-member handoff code is required for identity.
- Unknown numbers are onboarded with full name, email, preferred departure airport(s), and terms approval.
- If the supplied email already belongs to another account, NOVA refuses to attach the new phone from WhatsApp and directs the user to update the account securely on the website.
